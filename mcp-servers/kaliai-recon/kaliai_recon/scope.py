...
def current_engagement() -> Engagement:
    name = os.environ.get("KALIAI_ENGAGEMENT")
    dir_ = os.environ.get("KALIAI_ENGAGEMENT_DIR")
    if not name or not dir_:
        raise ScopeError(
            "No active engagement (KALIAI_ENGAGEMENT / KALIAI_ENGAGEMENT_DIR not set). "
            "Launch this server via `kaliai agent` after `kaliai engagement use <name>`."
        )
...
def log_call(engagement: Engagement | None, tool: str, target: str, args: list[str], verdict: str, detail: str = "") -> None:
    ...
    if engagement is None:
        ...
        print(f"kaliai-recon: {json.dumps(entry)}", flush=True)
        return
